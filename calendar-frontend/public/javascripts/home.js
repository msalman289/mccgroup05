    ...
    $.getJSON( '/eventlist', function( data ) {
		...
		if (data && data.length) {
			...
			$.each(data, function(){
				...
				tableContent += '<td><a href="#" class="linkeditevent" rel="' + this._id + '"><img src="http://130.233.42.186:8080/images/edit2.png" style="width:30px;height:30px;"/></a></td>';
				tableContent += '<td><a href="#" class="linkdeleteevent" rel="' + this._id + '"><img src="http://130.233.42.186:8080/images/delete2.png" style="width:30px;height:30px;"/></a></td>';
				tableContent += '</tr>';
				index++;
			});
...
function deleteEvent(event) {
    ...
    if (confirmation === true) {
        $.ajax({
            type: 'DELETE',
            url: 'http://130.233.42.186:8080/deleteevent/' + $(this).attr('rel')
        }).done(function( response ) {
